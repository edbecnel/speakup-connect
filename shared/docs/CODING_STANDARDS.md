...
All user-facing UI strings **must** be added to **`speakup_connect_app/lib/l10n/app_en.arb`** (US English source of truth) and referenced via `context.l10n` — never hardcoded in Dart widgets. Target locales (`app_ceb.arb`, `app_fil.arb`, …) are filled by translators later; developers only author English keys.
