...
The standard `google-services.json` stays at `speakup_connect_app/android/app/google-services.json`.
      ...
      - name: Restore git-ignored secrets
        run: |
          echo "${{ secrets.MONHS_GOOGLE_SERVICES_JSON }}" \
            > speakup_connect_app/android/app/src/monhs/google-services.json
          echo "${{ secrets.FIREBASE_OPTIONS_DART }}" \
            > speakup_connect_app/lib/config/firebase_options.dart
      - run: flutter pub get
        working-directory: speakup_connect_app
      ...
      - name: Restore git-ignored secrets
        run: |
          echo "${{ secrets.MONHS_GOOGLE_SERVICE_INFO_PLIST }}" \
            > speakup_connect_app/ios/Runner/GoogleService-Info-monhs.plist
          echo "${{ secrets.FIREBASE_OPTIONS_DART }}" \
            > speakup_connect_app/lib/config/firebase_options.dart
      - run: flutter pub get
        working-directory: speakup_connect_app
...
| `FIREBASE_OPTIONS_DART` | Contents of `speakup_connect_app/lib/config/firebase_options.dart` (shared) |
