 ...
 * Run: node speakup_connect_web/tools/translation-helper/generate-screen-route-aliases.js
 */
const fs = require('fs');
...
const ASSIGNABLE = path.join(
  REPO,
  'shared/functions/src/data/assignable_routes.json',
);
const OUT = path.join(REPO, 'shared/functions/src/data/screen_name_route_aliases.json');
...
function loadRouteConstants() {
  const text = fs.readFileSync(
    path.join(REPO, 'speakup_connect_app/lib/core/constants/route_constants.dart'),
    'utf8',
  );
...
function loadScreenRoutes(routeConstants) {
  const text = fs.readFileSync(
    path.join(REPO, 'speakup_connect_app/lib/core/router/app_router.dart'),
    'utf8',
  );
...
for (const file of walkScreens(path.join(REPO, 'speakup_connect_app/lib'))) {
