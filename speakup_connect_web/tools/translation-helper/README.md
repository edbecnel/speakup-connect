...
Web workspace for org admins and translation moderators to manage UI translations before exporting ARB files into `speakup_connect_app/lib/l10n/`. In-page help is built into `index.html` (expand **Translation workspace help** or click **Help** after sign-in).

**End-to-end workflow (canonical — start here):** [docs/INTERNATIONALIZATION.md §11](../../../shared/docs/INTERNATIONALIZATION.md#11-end-to-end-workflow-canonical)  
**GitHub issue:** [#48](https://github.com/edbecnel/speakup-connect/issues/48)  
**Architecture:** [docs/INTERNATIONALIZATION.md §13](../../../shared/docs/INTERNATIONALIZATION.md#13-translation-helper-tool)  
...
1. **Import** `speakup_connect_app/lib/l10n/app_en.arb` (English source keys) — platform `super_admin` only
...
| 2 | Deploy Cloud Functions (+ optional `shared/functions/.env` for AI) | PowerShell |
| 3a | Download `shared/scripts/service-account.json` | Firebase Console |
| 3b | Run `node shared/scripts/seed_roles.js` | PowerShell |
...
| **Step 3a–3d, 3b** — Admin scripts | **Service account JSON** file path | `shared\scripts\service-account.json` (not a password) |
...
- Your OpenAI key (`sk-…` — that goes in `shared/functions/.env` only)
...
From the repo `shared/functions/` folder. Choose **Option A** or **Option B** depending on whether you want AI draft.
...
# 2. Build and deploy (deploy reads shared/functions/.env)
npm run build
npx firebase-tools deploy --only functions:getTranslationWorkspaceAccess,functions:importTranslationSource,functions:importTranslationTargets,functions:listTranslationEntries,functions:saveTranslationEntry,functions:draftTranslation,functions:batchDraftTranslations,functions:exportTranslationArb,functions:listTranslationScreens,functions:createTranslationScreen,functions:updateTranslationScreen,functions:deleteTranslationScreen
```

Do **not** commit `shared/functions/.env`.
...
3. Save the downloaded JSON file to this exact path (use this file name exactly):
   ...
   ```
   D:\Dev\Speakup-Connect\shared\scripts\service-account.json
   ```
...
If you see `ENOENT: no such file or directory, open '...\shared\scripts\service-account.json'`, the file is missing or saved under a different name/path — repeat steps 1–3.
...
- `shared/scripts/service-account.json` in place (**Step 3a**).
...
cd D:\Dev\Speakup-Connect
$env:GOOGLE_APPLICATION_CREDENTIALS = "shared\scripts\service-account.json"
node shared/scripts/assign_super_admin.js your-firebase-auth-email@example.com
```
...
This file lets the Translation Helper sign in with **Firebase Auth** in the browser. It is **not** where your OpenAI key goes — that stays in `shared/functions/.env` (Step 2).

```powershell
cd D:\Dev\Speakup-Connect\speakup_connect_web\tools\translation-helper
...
Open `speakup_connect_web/tools/translation-helper/firebase-config.js` and fill in **only** the placeholders. The example file already has correct values for this project except the two marked `YOUR_*`:
...
**Common mistake:** putting your **OpenAI** key (`sk-…`) in `apiKey`. That field must be the Firebase **Web API Key** (`AIzaSy…`). OpenAI belongs only in `shared/functions/.env` → `TRANSLATION_AI_API_KEY`.
...
2. Click **Import `app_en.arb`** and choose `D:\Dev\Speakup-Connect\speakup_connect_app\lib\l10n\app_en.arb`.
...
See **[INTERNATIONALIZATION.md §11 — Phase E–G](../../../shared/docs/INTERNATIONALIZATION.md#phase-e--export-arb)** for export, `flutter gen-l10n`, hot restart, and commit.

1. Click **Export ARB**.
2. Save the JSON as `speakup_connect_app/lib/l10n/app_ceb.arb` or `speakup_connect_app/lib/l10n/app_fil.arb`.
3. Run from `D:\Dev\Speakup-Connect\speakup_connect_app`: `flutter gen-l10n`, hot restart the app, then commit ARB + generated Dart files.

### Step 12 — Human translator via Google Sheets (optional)

1. Click **Export CSV** for the target locale (or use `speakup_connect_app/lib/l10n/ceb_translations.csv` / `fil` equivalent in the repo).
2. **Populate the `screen` column** (required after `app_en.arb` changes or new keys):
   ...
   This scans Dart usage under `speakup_connect_app/lib/` and fills **`screen`** so reviewers know where each string appears. Re-run whenever English keys are added or moved. See [INTERNATIONALIZATION.md §11 — Populate screen column](../../../shared/docs/INTERNATIONALIZATION.md#populate-screen-column-for-reviewer-csv).
...
| Error | Fix |
|-------|-----|
| `Secret Manager API... disabled` (403) on deploy | Pull latest code — translation functions use `shared/functions/.env`, not Secret Manager. Redeploy after `git pull`. |
| `TRANSLATION_AI_API_KEY is not set` when using AI draft | Copy `shared/functions/.env.example` → `shared/functions/.env`, add your key, redeploy (see Step 2 Option B follow-up) |
...
- Put `TRANSLATION_AI_API_KEY` in `shared/functions/.env` (gitignored) — not in the app or git.
...
- Do not commit `firebase-config.js`, `shared/functions/.env`, or `shared/scripts/service-account.json`.

## Related scripts

| Script | Purpose |
|--------|---------|
| `shared/scripts/seed_roles.js` | Seed role definitions including `manageTranslations` |
| `shared/scripts/assign_admin.js` | Grant org admin for a Firebase Auth email |
| `shared/scripts/assign_super_admin.js` | Grant platform `super_admin` (Import English in web tool) |
