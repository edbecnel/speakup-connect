...
This project uses Cursor **Rules** for always-on constraints and Cursor **Skills** for repeatable, on-demand workflows. The goal is to keep chats small, reduce token usage, and prevent scope creep while staying consistent with `docs/`.
...
3. **Pointers** to existing docs/code (optional but very helpful):
   - docs: `@docs/DATABASE_DESIGN.md`, `@docs/CODING_STANDARDS.md`, etc.
   - code: `@lib/features/reminders/`, `@functions/src/index.ts`, etc.
...
- `Relevant docs: @docs/DATABASE_DESIGN.md @docs/ARCHITECTURE.md`
- `Relevant code: @lib/features/groups/ @functions/src/group_membership.ts`
